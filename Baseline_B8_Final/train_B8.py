...
from eval_final_baseline import Person_Classifer , Group_Classifer , collate_group_fn
...
if __name__ == '__main__':
    ...
    print("Loading Baseline 8 Training Configuration...")
    ...
    logger= setup_logger()
    logger.info("Resume Baseline_B8 Training")
    ...
    train_preprocess = albu.Compose([
        ...
        albu.OneOf([ albu.GaussianBlur(blur_limit=(3, 9)), albu.ColorJitter(brightness=0.2),
            albu.RandomBrightnessContrast(),albu.GaussNoise()], p=0.7),
    ...
    Person=Person_Classifer(num_classes=9)
    Person.load_state_dict(torch.load('/kaggle/input/notebooke003360755/best_Baseline_B7_A_model.pth'))
    ...
    if config.training['group_activity']['use_scheduler']:
        if config.training['group_activity']['scheduler_type'] == 'ReduceLROnPlateau':
            ...
            scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
                optimizer,
                T_0=config.training['group_activity']['scheduler_T_0'],
                T_mult=config.training['group_activity']['scheduler_T_mult'],
                eta_min=0.000000001
            )
        else:
    ...
    train_dataset = GroupActivityTempDataset(
        videos_path="/kaggle/input/volleyball/volleyball_/videos",
        annot_path='/kaggle/input/b5-banno/annot_all.pkl',
        split=train_split,
        sort=True,
        transform=train_preprocess)

    val_dataset = GroupActivityTempDataset(
        videos_path="/kaggle/input/volleyball/volleyball_/videos",
        annot_path='/kaggle/input/b5-banno/annot_all.pkl',
        split=val_split,
        sort=True,
    ...
    for epoch in range(start_epoch,num_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            logger.info(f"Best model saved to: best_Baseline_B8_model.pth")
