...
from data.data_loader import GroupActivityTempDataset
...
CONFIG_PATH="configs/b8_config.yaml"
...
class Person_Classifer(nn.Module):
    def __init__(self,num_classes=9):
        ...
        self.lstm1 = nn.LSTM(2048,self.hidden_size,num_layers=self.num_layers,batch_first=True)
    ...
    def forward(self, x):  # x: (B, 9, 3, H, W)
        ...
        feats = self.resnet50(x)  # (B*T*seq, 2048, 1, 1)

        x = feats.view(B*bb,seq, -1)  #(B*T,seq,2048)
        
        x = self.layer_norm(x)

        x , (h,c) =self.lstm1(x)  # ( B * T , seq , 512)
...
class Group_Classifer(nn.Module):
    def __init__(self, Person_Classifer , num_classes=8):
        ...
        self.resnet50=Person_Classifer.resnet50

        self.lstm1 = Person_Classifer.lstm1

        for module in [self.resnet50, self.lstm1]:
        ...
        self.lstm2=nn.LSTM(2048,self.hidden_size,num_layers=self.num_layers,batch_first=True,dropout=0.5)
    ...
    def forward(self, x):  # x: (B, T, seq , 3, H, W)
        ...
        feats = self.resnet50(x)  # (B*bb*seq, 2048, 1, 1)

        feats = feats.view(B*seq,bb, -1)  #(B*bb,seq,2048)
        feats = self.layer_norm(feats) #(B*bb,seq,2048)

        x2 , _ = self.lstm1(feats) #(B*bb,seq,512)

        x = torch.cat([feats,x2],dim=2)
        x=x.contiguous()

        x = x.view(B*seq, bb, -1) # (B * seq, bb, 512)

        first_team = x[:, :6, :]  # (B * seq, 6, 512)
        second_team = x[:, 6:, :]  # (B * seq, 6, 512)

        first_team = self.pool(first_team)  # (B * seq, 1, 1024)
        second_team = self.pool(second_team)  # (B * seq, 1, 1024)

        x = torch.cat([first_team, second_team], dim=2)  # (B * seq, 2, 2048)
       
        x = x.view(B, seq, -1) # (B, seq, 2048)
        x = self.layer_norm(x)
        x,_= self.lstm2(x) # (B, seq, 512)

        x = x[:, -1, :] # (B, 512)
        x = self.classifier(x)  # (B, num_class)
...
if __name__ == '__main__':
    # Load B8 Configuration
    print("Loading Baseline 8 Configuration...")
    config = load_yaml_config(CONFIG_PATH)
    
    # Extract configuration parameters
    root_dataset = config.data['dataset_root']
    model_path = os.path.join(config.evaluation['results_dir'], 'best_best_Baseline_B8_model.pth')
    ...
    test_dataset = GroupActivityTempDataset(
    ...
    Average Loss: 0.4142
    Overall F1 Score (Macro): 0.9095
    Overall F1 Score (Weighted): 0.9047
